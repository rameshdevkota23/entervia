...
class Entervia_Admin_Info
{
    ...
    {
        add_theme_page(
            esc_html__('entervia Theme', 'entervia'),
            esc_html__('entervia Theme', 'entervia'),
            'edit_theme_options',
            'entervia',
    ...
    public function check_rt_easy_builder_freemius()
    {
        if (function_exists('RT_Easy_Builder\rt_freemius') && ! RT_Easy_Builder\rt_freemius()->is_not_paying()) {
            ...
            add_action('wp_ajax_dismiss_entervia_notification', array($this, 'dismiss_notification_ajax_handler'));
        }
    }
    ...
    {
        delete_transient('entervia_notification_dismissed');
    }
    ...
    public function compatible_check()
    {
        ...
        $dismissed = get_option('entervia_notification_dismissed');
        if (!$dismissed) {
    ...
    public function dismiss_notification()
    {
        ...
        update_option('entervia_notification_dismissed', true);
    }
            ...
            (function($) {
                $(document).on('click', '.notice.is-dismissible .notice-dismiss', function() {
                    $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action: 'dismiss_entervia_notification',
...
// Instantiate the entervia Admin Info class
new Entervia_Admin_Info();
